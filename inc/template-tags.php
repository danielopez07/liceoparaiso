 ...
 */
function liceoparaiso_using_amp_live_list_comments() {
 ...
 */
function liceoparaiso_add_amp_live_list_pagination_attribute( $markup ) {
 ...
 */
function liceoparaiso_index_header() {
	if ( is_home() && ! is_front_page() ) {
		...
		<header class="page-header">
			...
			<?php
				...
				printf( esc_html__( 'Search Results for: %s', 'liceoparaiso' ), '<span>' . get_search_query() . '</span>' );
			?>
			</h1>
		</header><!-- .page-header -->
		<?php
 ...
 */
function liceoparaiso_posted_on() {
	...
	$posted_on = sprintf(
		...
		esc_html_x( 'Posted on %s', 'post date', 'liceoparaiso' ),
 ...
 */
function liceoparaiso_posted_by() {
	$byline = sprintf(
		/* translators: %s: post author. */
		esc_html_x( 'by %s', 'post author', 'liceoparaiso' ),
 ...
 */
function liceoparaiso_post_categories() {
	// Only show categories on post types that have categories.
	if ( 'post' === get_post_type() ) {
		/* translators: used between list items, there is a space after the comma */
		$categories_list = get_the_category_list( esc_html__( ', ', 'liceoparaiso' ) );
		if ( $categories_list ) {
			/* translators: 1: list of categories. */
			printf( '<span class="cat-links">' . esc_html__( 'Posted in %1$s', 'liceoparaiso' ) . ' </span>', $categories_list ); // WPCS: XSS OK.
		}
	}
}
 ...
 */
function liceoparaiso_post_tags() {
	// Only show tags on post types that have categories.
	if ( 'post' === get_post_type() ) {
		/* translators: used between list items, there is a space after the comma */
		$tags_list = get_the_tag_list( '', esc_html_x( ', ', 'list item separator', 'liceoparaiso' ) );
		if ( $tags_list ) {
			/* translators: 1: list of tags. */
			printf( '<span class="tags-links">' . esc_html__( 'Tagged %1$s', 'liceoparaiso' ) . ' </span>', $tags_list ); // WPCS: XSS OK.
		}
	}
}
 ...
 */
function liceoparaiso_comments_link() {
		...
		comments_popup_link(
			sprintf(
				wp_kses(
					...
					__( 'Leave a Comment<span class="screen-reader-text"> on %s</span>', 'liceoparaiso' ),
					array(
						'span' => array(
							'class' => array(),
						),
 ...
 */
function liceoparaiso_edit_post_link() {
	edit_post_link(
		sprintf(
			wp_kses(
				/* translators: %s: Name of current post. Only visible to screen readers */
				__( 'Edit <span class="screen-reader-text">%s</span>', 'liceoparaiso' ),
				array(
					'span' => array(
						'class' => array(),
					),
 ...
 */
function liceoparaiso_post_thumbnail() {
 ...
 */
function liceoparaiso_attachment_in( $post ) {
	if ( ! empty( $post->post_parent ) ) :
		$postlink = sprintf(
			/* translators: %s: original post where attachment was added. */
			esc_html_x( 'in %s', 'original post', 'liceoparaiso' ),
 ...
 */
function liceoparaiso_the_attachment_navigation() {
	?>
	<nav class="navigation post-navigation" role="navigation">
		<h2 class="screen-reader-text"><?php echo esc_html__( 'Post navigation', 'liceoparaiso' ); ?></h2>
		<div class="nav-links">
			<div class="nav-previous">
				<div class="post-navigation-sub">
					<?php echo esc_html__( 'Previous attachment:', 'liceoparaiso' ); ?>
				</div>
				<?php previous_image_link( false ); ?>
			</div><!-- .nav-previous -->
			<div class="nav-next">
				<div class="post-navigation-sub">
					<?php echo esc_html__( 'Next attachment:', 'liceoparaiso' ); ?>
				</div>
